...
allPawnReplacementTests = TestList [
    ...
    TestLabel "No Replacing" testNRP,
    TestLabel "PieceCount No Replacing" testPNRP
    ]
...
nof = addPawn safeKings (4,3) W
-- Expted Value: 
-- 3 Moves from King 
-- 1 Move from Pawn
testNRP = 4 ~=? length ( validMoves nof  W )
-- There will never be more than 3 Figures if i replace one
testPNRP = True ~=? (all ((==) 3) $ map (length) ( validMoves nof  W ))
